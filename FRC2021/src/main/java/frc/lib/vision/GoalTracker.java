...
/**
 * This is used in the event that multiple goals are detected to judge all goals based on timestamp,
 * stability, and continuation of previous goals (i.e. if a goal was detected earlier and has
 * changed locations). This allows the robot to make consistent decisions about which goal to aim at
 * and to smooth out jitter from vibration of the camera.
 *
 * @see GoalTrack
 */
public class GoalTracker {
  ...
  /**
   * TrackReportComparators are used in the case that multiple tracks are active (e.g. we see or
   * have recently seen multiple goals). They contain heuristics used to pick which track we should
   * aim at by calculating a score for each track (highest score wins).
   */
