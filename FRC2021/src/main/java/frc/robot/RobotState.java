...
public class RobotState {
  ...
  private Translation2d getCameraToVisionTargetPose(
      TargetInfo target, boolean high, Limelight source) {
    // Compensate for camera pitch
    Translation2d xz_plane_translation =
        new Translation2d(target.getX(), target.getZ()).rotateBy(source.getHorizontalPlaneToLens());
    ...
    double differential_height =
        source.getLensHeight()
            - (high ? Constants.kPortTargetHeight : Constants.kHatchTargetHeight);
  ...
  private void updatePortGoalTracker(
      double timestamp,
      List<Translation2d> cameraToVisionTargetPoses,
      GoalTracker tracker,
      Limelight source) {
    ...
    Pose2d cameraToVisionTarget =
        Pose2d.fromTranslation(
            cameraToVisionTargetPoses.get(0).interpolate(cameraToVisionTargetPoses.get(1), 0.5));

    Pose2d fieldToVisionTarget =
        getFieldToTurret(timestamp)
            .transformBy(source.getTurretToLens())
  ...
  public synchronized void addVisionUpdate(
      double timestamp, List<TargetInfo> observations, Limelight source) {
  ...
  // use known field target orientations to compensate for inaccuracy, assumes robot starts pointing
  // directly away
  // from and perpendicular to alliance wall
  private final double[] kPossibleTargetNormals = {
    0.0, 90.0, 180.0, 270.0, 30.0, 150.0, 210.0, 330.0
  };
  ...
  public synchronized Optional<AimingParameters> getAimingParameters(
      boolean highTarget, int prev_track_id, double max_track_age) {
    GoalTracker tracker = highTarget ? vision_target_high_ : vision_target_low_;
    ...
    // Find the best track.
    TrackReportComparator comparator =
        new TrackReportComparator(
