...
public class PathPlanner {
  ...
  public static void print(double[][] path) {
    ...
    for (double[] u : path) System.out.println(u[0] + "\t" + u[1]);
  }
  ...
  public double[][] convertArray(double[][] array) {
    ...
    for (int i = 0; i < array.length; i++) {
      ...
      temp[i][1] =
          ((array[i][1])
              * Constants.kDriveTicksPerFoot
              * Constants.kTalonUpdateTime); // .1 = sec to ms
    }
  ...
  public double[][] smoother(
      double[][] path, double weight_data, double weight_smooth, double tolerance) {
  ...
  public int[] injectionCounter2Steps(
      double numNodeOnlyPoints, double maxTimeToComplete, double timeStep) {
    int first = 0;
    int second = 0;
    int third = 0;
  ...
  public void leftRight(double[][] smoothPath, double robotTrackWidth) {
    ...
    for (int i = 0; i < smoothPath.length - 1; i++) {
      gradient[i][1] =
          Math.atan2(
              smoothPath[i + 1][1] - smoothPath[i][1], smoothPath[i + 1][0] - smoothPath[i][0]);
    }
    ...
    for (int i = 0; i < gradient.length; i++) {
      leftPath[i][0] =
          (robotTrackWidth / 2 * Math.cos(gradient[i][1] + Math.PI / 2)) + smoothPath[i][0];
      leftPath[i][1] =
          (robotTrackWidth / 2 * Math.sin(gradient[i][1] + Math.PI / 2)) + smoothPath[i][1];
      ...
      // convert to degrees 0 to 360 where 0 degrees is +X - axis, accumulated to aline with WPI
      // sensor
  ...
  public void calculate(double totalTime, double timeStep, double robotTrackWidth) {
    ...
    mSmoothCenterVelocity =
        smoother(mSmoothCenterVelocity, mVelocityAlpha, mVelocityBeta, mVelocityTolerance);
    mSmoothLeftVelocity =
        smoother(mSmoothLeftVelocity, mVelocityAlpha, mVelocityBeta, mVelocityTolerance);
    mSmoothRightVelocity =
        smoother(mSmoothRightVelocity, mVelocityAlpha, mVelocityBeta, mVelocityTolerance);
