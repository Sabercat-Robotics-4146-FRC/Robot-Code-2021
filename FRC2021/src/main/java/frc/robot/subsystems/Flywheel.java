...
public class Flywheel extends Subsystem {
  ...
  private Flywheel() {
    ...
    TalonSRXUtil.checkError(
        mLeader.configSelectedFeedbackSensor(
            FeedbackDevice.CTRE_MagEncoder_Relative, 0, Constants.kLongCANTimeoutMs),
        "Flywheel: Could not detect encoder: ");

    // set gains
    TalonSRXUtil.checkError(
        mLeader.config_kP(0, Constants.kFlywheelKp, Constants.kLongCANTimeoutMs),
        "Flywheel: could not set kP: ");
    TalonSRXUtil.checkError(
        mLeader.config_kI(0, Constants.kFlywheelKi, Constants.kLongCANTimeoutMs),
        "Flywheel: could not set kI: ");
    TalonSRXUtil.checkError(
        mLeader.config_kD(0, Constants.kFlywheelKd, Constants.kLongCANTimeoutMs),
        "Flywheel: could not set kD: ");
    TalonSRXUtil.checkError(
        mLeader.config_kF(0, Constants.kFlywheelKf, Constants.kLongCANTimeoutMs),
        "Flywheel: Could not set kF: ");

    mFollower =
        TalonSRXFactory.createPermanentSlaveTalon(
            Constants.kFlywheelFollowerId, Constants.kFlywheelLeaderId);
  }
