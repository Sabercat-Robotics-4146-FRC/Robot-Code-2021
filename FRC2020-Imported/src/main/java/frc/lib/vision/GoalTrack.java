...
/**
 * A class that is used to keep track of all goals detected by the vision system. As goals are
 * detected/not detected anymore by the vision system, function calls will be made to create,
 * destroy, or update a goal track.
 *
 * <p>This helps in the goal ranking process that determines which goal to fire into, and helps to
 * smooth measurements of the goal's location over time.
 *
 * @see GoalTracker
 */
public class GoalTrack {
  ...
  public static synchronized GoalTrack makeNewTrack(
      double timestamp, Pose2d first_observation, int id) {
  ...
  public synchronized boolean tryUpdate(double timestamp, Pose2d new_observation) {
    ...
    double distance =
        mSmoothedPosition.inverse().transformBy(new_observation).getTranslation().norm();
